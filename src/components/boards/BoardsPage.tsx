...
export function BoardsPage({ user }: BoardsPageProps) {
  ...
  const handleCreate = () => {
    if (!canCreate) {
      toast.error("Upgrade membership untuk membuat board lebih banyak.");
      return;
    }
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-muted-foreground font-body text-sm">Belum ada board. Buat board pertama!</p>
            </div>
          ) : (
            boards?.map((board) => (
              <motion.div
