      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground font-body mt-1">All registered users.</p>
          </div>
        ...
        <div className="mt-4 space-y-2">
          {isLoading ? (
            ...
            filtered?.map((profile) => {
              ...
              return (
                <div key={profile.id} className="bg-card border border-border rounded-lg p-4 flex items-center justify-between">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    {role === "admin" && <Badge variant="destructive" className="text-[10px] uppercase">Admin</Badge>}
                    {membership?.is_active ? (
                      <Badge className="bg-primary/20 text-primary text-[10px] uppercase">Member</Badge>
                    ) : (
