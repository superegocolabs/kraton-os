...
export function ProjectsPage({ user }: ProjectsPageProps) {
  ...
  const addProject = useMutation({
    mutationFn: async (values: { name: string; description: string; client_id: string | null; budget: number; status: string; start_date: string | null; end_date: string | null }) => {
  ...
  const updateProject = useMutation({
    mutationFn: async ({ id, ...values }: { id: string } & Partial<Tables<"projects">>) => {
      ...
      <div className="p-4 md:p-6 lg:p-8 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
          ...
          <ProjectDetail
            ...
            clients={clients ?? []}
            onUpdate={(values) => updateProject.mutate({ id: selectedProject.id, ...values })}
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
        ...
        <div className="mt-4">
          <ProjectList projects={filtered ?? []} isLoading={isLoading} onSelect={setSelectedProjectId} />
        </div>
      </motion.div>
      ...
      <AddProjectDialog
        ...
        onSubmit={(values) => addProject.mutate(values)}
